...
export default function Login() {
  ...
  // Parse Query Param (e.g., ?redirect=checkout)
  ...
  useEffect(() => {
    if (userInfo) {
      if (userInfo.isAdmin) {
        ...
        // If redirect is 'checkout', go there. Else go home.
    ...
    <div className="min-h-screen flex bg-white pt-20">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 md:p-16 bg-white relative">
        <div className="w-full max-w-md animate-fade-up">
          <div className="mb-10">
            <h1 className="text-4xl font-serif text-[#1C1917] mb-3">
              Welcome Back
            </h1>
            <p className="text-gray-500">
              New to Beads & Bloom?{" "}
              {/* Maintain the redirect param when going to Signup */}
              <Link
                to={redirect === 'checkout' ? `/signup?redirect=checkout` : '/signup'}
                className="text-[#FF2865] font-bold hover:underline"
              >
                Create an account
              </Link>
            </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <Link
                  to="/forgot-password"
                  className="text-xs text-gray-400 hover:text-[#FF2865]"
                >
                  Forgot?
                </Link>
              </div>
          ...
          {redirect === 'checkout' && (
            <div className="mt-8 pt-8 border-t border-gray-100 text-center animate-fade-in">
              <p className="text-gray-500 text-sm mb-4">
                Don't want to create an account yet?
              </p>
              <button
                onClick={() => navigate('/checkout')}
                className="w-full bg-white border border-gray-200 text-gray-700 py-4 rounded-xl font-bold uppercase tracking-[0.2em] hover:bg-gray-50 hover:border-gray-400 transition-all duration-300 flex items-center justify-center gap-3"
              >
                Continue as Guest
