...
import { 
  ...
  ArrowRight, ShieldCheck, MapPin, Lock, AlertCircle, ShoppingBag, Tag, Loader2, Mail, Copy 
} from 'lucide-react';
...
export default function Checkout() {
  ...
  // Combined Form Data (Includes Email for Guests)
  ...
  // --- CALCULATIONS ---
  const { itemsPrice, shippingCost, taxPrice, discountAmount, finalTotal } = useMemo(() => {
    ...
    return { itemsPrice, shippingCost, taxPrice, discountAmount, finalTotal };
  }, [totalAmount, coupon]);
  ...
  useEffect(() => {
    return () => { if(step === 3) dispatch(resetOrder()); };
  }, [dispatch, step]);
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center text-center px-6 animate-fade-up">
        ...
        <p className="text-gray-500 max-w-md mb-8 leading-relaxed">
          Your order has been confirmed. We've sent a receipt to <span className="font-bold text-gray-800">{formData.email}</span>.
        </p>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-8 flex items-center justify-between gap-4 max-w-xs w-full shadow-sm hover:shadow-md transition-shadow">
            <div className="text-left">
                <p className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-1">Order ID</p>
                <p className="font-mono text-sm font-bold text-[#1C1917] break-all">{order._id}</p>
            </div>
            <button 
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md">
            {userInfo && (
                <Link to="/myorders" className="flex-1 bg-[#1C1917] text-white px-8 py-4 rounded-full font-bold uppercase tracking-widest text-xs hover:bg-[#FF2865] transition-all shadow-lg text-center">
                View Order
                </Link>
            )}
            <Link to="/shop" className="flex-1 bg-white border border-gray-200 text-[#1C1917] px-8 py-4 rounded-full font-bold uppercase tracking-widest text-xs hover:bg-gray-50 hover:border-gray-300 transition-all text-center">
            Continue Shopping
            </Link>
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          ...
          <div className="w-full lg:w-2/3">
              ...
              <form onSubmit={handleNextStep} className="bg-white p-6 md:p-10 rounded-3xl shadow-sm border border-gray-100 animate-fade-up">
                ...
                <div className="mb-8">
                   <div className="flex items-center gap-3 mb-6 border-b border-gray-100 pb-4">
                      ...
                      <h2 className="text-2xl font-serif text-[#1C1917]">Contact Info</h2>
                   </div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                             ...
                             <input 
                                ...
                                disabled={!!userInfo} // Lock if logged in
                   ...
                   {!userInfo && (
                       <p className="text-xs text-gray-400 mt-3">
                           Already have an account? <Link to="/login?redirect=checkout" className="text-[#FF2865] underline">Log in</Link>
                       </p>
                   )}
                </div>
                ...
                <div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <input type="text" name="city" required value={formData.city} onChange={handleInputChange} className="w-full bg-[#F9F8F6] border-0 rounded-xl px-5 py-3 outline-none" placeholder="City" />
                            <div className="relative">
                                <select name="state" required value={formData.state} onChange={handleInputChange} className="w-full bg-[#F9F8F6] border-0 rounded-xl px-5 py-3 outline-none appearance-none">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <input type="text" name="postalCode" required value={formData.postalCode} onChange={handleInputChange} className="w-full bg-[#F9F8F6] border-0 rounded-xl px-5 py-3 outline-none" placeholder="Pincode" />
                            <input type="text" name="phoneNumber" required value={formData.phoneNumber} onChange={handleInputChange} className="w-full bg-[#F9F8F6] border-0 rounded-xl px-5 py-3 outline-none" placeholder="Phone" maxLength={10} />
                        </div>
                    </div>
                </div>

                <div className="flex items-center justify-between pt-6 border-t border-gray-100 mt-8">
                    <Link to="/cart" className="flex items-center gap-2 text-sm text-gray-500 hover:text-[#1C1917]"><ArrowLeft className="w-4 h-4" /> Return to Cart</Link>
                    <button type="submit" className="bg-[#1C1917] text-white px-8 py-3 rounded-full font-bold uppercase text-xs hover:bg-[#FF2865] flex items-center gap-2">Proceed to Payment <ArrowRight className="w-4 h-4" /></button>
                </div>
              </form>
            )}
            ...
            {step === 2 && (
              <div className="bg-white p-6 md:p-10 rounded-3xl shadow-sm border border-gray-100 animate-fade-up">
                  {error && <div className="bg-red-50 text-red-500 p-4 rounded-xl mb-6 text-sm flex gap-2"><AlertCircle className="w-4 h-4" /> {error}</div>}
                  ...
                  <div className="bg-[#F9F8F6] p-4 rounded-xl mb-6 text-sm text-gray-600 space-y-1">
                     <p><span className="font-bold text-[#1C1917]">Contact:</span> {formData.email}</p>
                     <p><span className="font-bold text-[#1C1917]">Ship to:</span> {formData.address}, {formData.city}, {formData.postalCode}</p>
                  </div>

                  <div className="border-2 border-[#1C1917] bg-gray-50 rounded-2xl p-6 mb-8 relative">
                     <div className="absolute right-0 top-0 bg-[#1C1917] text-white text-[10px] font-bold uppercase px-3 py-1 rounded-bl-xl">Recommended</div>
                     <div className="flex items-center gap-4">
                         <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm"><Smartphone className="w-6 h-6 text-[#1C1917]" /></div>
                         <div>
                             <h3 className="font-bold text-lg text-[#1C1917]">Razorpay Secure</h3>
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-gray-100">
                    <button onClick={() => setStep(1)} className="flex items-center gap-2 text-sm text-gray-500 hover:text-[#1C1917]"><ArrowLeft className="w-4 h-4" /> Edit Details</button>
                    <button 
          ...
          <div className="w-full lg:w-1/3 sticky top-32">
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-xl border border-gray-100">
              <h3 className="font-serif text-xl text-[#1C1917] mb-6 flex items-center gap-2"><ShoppingBag className="w-5 h-5" /> Order Summary</h3>
              <div className="space-y-4 mb-6 max-h-64 overflow-y-auto pr-2 scrollbar-thin">
                {cartItems.map((item) => (
                  <div key={`${item.id}-${item.size}`} className="flex gap-4">
                    ...
                    <div className="text-right text-sm font-bold">₹{(item.price * item.quantity).toLocaleString()}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between text-gray-600"><span>Shipping</span>{shippingCost === 0 ? <span className="text-green-600 font-bold">Free</span> : <span>₹{shippingCost}</span>}</div>
                <div className="flex justify-between text-gray-600"><span>Taxes (5%)</span><span>₹{taxPrice}</span></div>
                {coupon && (
              ...
              <div className="h-[1px] bg-gray-100 my-4"></div>
              <div className="flex justify-between text-xl font-bold text-[#1C1917] mb-6"><span>Total</span><span>₹{finalTotal.toLocaleString()}</span></div>
              <div className="bg-green-50 p-4 rounded-xl flex items-center gap-3 text-green-800 text-xs font-bold border border-green-100">
                 <ShieldCheck className="w-5 h-5 flex-shrink-0" /><span>SSL Encrypted Payment</span>
              </div>
            </div>
          </div>
