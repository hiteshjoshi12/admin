    ...
    return (
      <div className="min-h-screen flex items-center justify-center flex-col gap-4">
        ...
        <button
          ...
          className="underline text-[#1C1917]"
        >
          Try Again
        </button>
      </div>
    ...
    <div className="bg-[#F9F8F6] min-h-screen pt-20">
      ...
      <div className="bg-white border-b border-gray-100 py-12 px-6 text-center">
        ...
        <p className="text-gray-500 text-sm max-w-lg mx-auto">
          Handcrafted luxury for every occasion.
        </p>
      </div>
          ...
          {!loading && products.length === 0 && (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <button
                ...
                className="text-[#FF2865] underline font-medium"
              >
                Clear Filters
              </button>
            </div>
    ...
    >
      <div className="relative aspect-[3/4] overflow-hidden rounded-xl bg-gray-100 mb-4">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
          {/* SOLD OUT */}
          {isOutOfStock ? (
            <span className="bg-[#1C1917] text-white px-3 py-1 text-[10px] font-bold uppercase tracking-widest shadow-sm">
              Sold Out
            </span>
          ) : (
            <>
              {/* NEW ARRIVAL - Based on your checkbox */}
              {product.isNewArrival && (
                <span className="bg-white text-[#1C1917] px-3 py-1 text-[10px] font-bold uppercase tracking-widest shadow-sm">
                  New
                </span>
              )}

              {/* DISCOUNT BADGE - Only shows if math results in a discount */}
              {hasDiscount && discountPercent > 0 && (
                <span className="bg-[#FF2865] text-white px-3 py-1 text-[10px] font-bold uppercase tracking-widest shadow-sm">
                  {discountPercent}% OFF
                </span>
              )}
            </>
      ...
      {/* --- PRODUCT DETAILS --- */}
      <h3 className="font-serif text-lg text-[#1C1917] group-hover:text-[#FF2865] transition-colors truncate">
        {product.name}
      </h3>
