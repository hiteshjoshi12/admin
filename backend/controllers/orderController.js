...
const addOrderItems = async (req, res) => {
  ...
  try {
    ...
    // OPTIONAL: Send email immediately if COD
...
// @desc    Update order to delivered (Legacy)
// @route   PUT /api/orders/:id/deliver
// @access  Private/Admin
const updateOrderToDelivered = async (req, res) => {
  try {
...
const trackOrderPublic = async (req, res) => {
  ...
  try {
    ...
    // Check if order exists AND user exists AND email matches
    if (order && order.user && order.user.email.toLowerCase() === email.toLowerCase().trim()) {
      res.json({
        _id: order._id,
        createdAt: order.createdAt,
        ...
        deliveredAt: order.deliveredAt,
        awbCode: order.awbCode, 
        courierName: order.courierCompanyName || "Shiprocket", // Show real courier
        orderItems: order.orderItems, 
        totalPrice: order.totalPrice
      });
    } else {
...
// @desc    Create Razorpay Order ID (With Atomic Lock)
// @route   POST /api/orders/:id/pay/initiate
// @access  Private
const initiateRazorpayPayment = async (req, res) => {
  try {
    ...
    // 2. IMMEDIATE LINKING: Save the Razorpay Order ID to DB instantly
    order.razorpayOrderId = razorpayOrder.id;
    await order.save(); 
...
const verifyRazorpayPayment = async (req, res) => {
  ...
  try {
    ...
    if (expectedSignature === razorpaySignature) {
      ...
      // Fetch full order with user details to send personalized email
      const fullOrder = await Order.findById(updatedOrder._id).populate('user', 'name email');
      await sendOrderConfirmation(fullOrder); // Send Email Logic
      // -------------------------------
...
const handleRazorpayWebhook = async (req, res) => {
  ...
  if (digest === req.headers['x-razorpay-signature']) {
    ...
    if (event === 'payment.captured') {
        const payment = req.body.payload.payment.entity;
        const razorpayOrderId = payment.order_id; // The ID we saved during Initiate
...
module.exports = {
  ...
  getOrders,
  trackOrderPublic,
  shipOrder,          // <--- Shiprocket
  updateOrderStatus,  // <--- Manual Update
  initiateRazorpayPayment, // <--- Razorpay Init
  verifyRazorpayPayment,   // <--- Razorpay Verify
  handleRazorpayWebhook    // <--- Webhook
};