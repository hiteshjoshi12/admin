...
const { sendOrderConfirmation, sendOrderStatusEmail } = require('../utils/sendEmail');
...
const addOrderItems = async (req, res) => {
  const {
    ...
    shippingAddress,
    paymentMethod,
    itemsPrice, // Optional from frontend (we ignore for security mostly)
    taxPrice: frontendTax,
    shippingPrice: frontendShipping,
    totalPrice: frontendTotal,
    // Optional guest details if passed separately, otherwise we take from shipping
    guestEmail
  } = req.body;
  ...
  try {
    ...
    for (const item of orderItems) {
      ...
      if (!sizeVariant) throw new Error(`Size ${item.size} invalid for ${item.name}`);
      if (sizeVariant.quantity < item.quantity) {
        throw new Error(`Out of stock: ${item.name} (Size ${item.size})`);
      }
      ...
      // Add to Total (Using DB Price)
      calculatedItemsPrice += dbProduct.price * item.quantity;
    }

    // 3. CALCULATE FINAL TOTALS
    // Logic: Free shipping over 5000, else 150
    const shippingPrice = calculatedItemsPrice > 5000 ? 0 : 150; 
    const taxPrice = Math.round(calculatedItemsPrice * 0.05); // 5% Tax
    
    const discountAmount = 0; // Placeholder for coupon logic

    const totalPrice = (calculatedItemsPrice + taxPrice + shippingPrice) - discountAmount;
    ...
    const orderData = {
      ...
      taxPrice,
      shippingPrice,
      totalPrice, // Secure Total
      isPaid: false,
      isDelivered: false,
      orderStatus: 'Processing',
    };

    // 🚨 BRANCH: Logged In vs Guest
    if (req.user && req.user._id) {
       // A. Logged In User
       orderData.user = req.user._id;
    } else {
       // B. Guest User
       // We assume the shipping address or a separate field has the contact info
       orderData.guestInfo = {
          name: req.body.name || shippingAddress.name || "Guest", 
          email: req.body.email || guestEmail || "guest@example.com" // Ensure you send 'email' from frontend checkout
       };
    }
    ...
    if (paymentMethod === 'cod') {
        // If guest, 'user' populate will be null, that's fine.
    ...
    const status = error.message.includes("found") || error.message.includes("stock") ? 400 : 500;
  ...
  try {
    ...
    if (shippingData) {
      ...
      if (shippingData.awbCode) {
          ...
          console.log("Order pushed to Shiprocket. Waiting for courier assignment.");
      }
...
const trackOrderPublic = async (req, res) => {
  ...
  try {
    ...
    if (order && userEmail && userEmail.toLowerCase() === email.toLowerCase().trim()) {
      res.json({
        _id: order._id,
...
const verifyRazorpayPayment = async (req, res) => {
  ...
  try {
    ...
    if (expectedSignature === razorpaySignature) {
      ...
      const email = order.user?.email || order.guestInfo?.email || "guest@example.com";
...
const handleShiprocketWebhook = async (req, res) => {
  try {
    ...
    console.log(`🚚 Shiprocket Update for Order ${order_id}: ${current_status}`);
    ...
    if (order) {
      ...
      else if (current_status === 'CANCELED' || current_status === 'RTO INITIATED') {
